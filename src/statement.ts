...
export class Statement {
  ...
  constructor(stmt: HanaStatement) {
    this.stmt = stmt;
  }
  ...
  public async exec<T>(params?: any | any[]): Promise<T> {
    ...
    return new Promise((resolve, reject) => {
      const myFn = (err: Error, results: T) => {
        if (err) {
          return reject(err);
        }
        resolve(results);
      ...
      params
        ? this.stmt.exec<T>(Array.isArray(params) ? params : [params], myFn)
        : this.stmt.exec<T>(myFn);
    });
  }
  ...
  public async execQuery<T>(params?: any | any[]): Promise<T[]> {
    ...
    return new Promise((resolve, reject) => {
      this.stmt.execQuery<T>(params, (err: Error, rs: ResultSet) => {
        if (err) {
          return reject(err);
        }
        const rows = [];
  ...
  public async execBatch<T>(params: any[]): Promise<T> {
    ...
    return new Promise((resolve, reject) => {
      this.stmt.execBatch<T>(params, (err, results) => {
        if (err) {
          return reject(err);
        }
        resolve(results);
